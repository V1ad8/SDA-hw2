...
lru_cache *init_lru_cache(unsigned int cache_capacity)
{
	...
	cache->order = dll_create(sizeof(void *));
...
bool lru_cache_put(lru_cache *cache, void *key, void *value, void **evicted_key)
{
	...
	if (ht_has_key(cache->ht, key)) {
		dll_node_t *node = move_node_to_end(cache->order, info);
	...
	if (lru_cache_is_full(cache)) {
		*evicted_key = ((info_t *)cache->order->head->data)->key;
		dll_remove_nth_node(cache->order, 0);
		ht_remove_entry(cache->ht, *evicted_key);
	}

	dll_node_t *node = dll_add_nth_node(cache->order, cache->order->size, info);
	ht_put(cache->ht, key, sizeof(key), node, sizeof(node));
...
{
	if (ht_has_key(cache->ht, key)) {
		dll_node_t *node = ht_get(cache->ht, key);
...
void lru_cache_remove(lru_cache *cache, void *key)
{
	if (ht_has_key(cache->ht, key)) {
		dll_node_t *node = ht_get(cache->ht, key);
		free(dll_remove_node(cache->order, node));
